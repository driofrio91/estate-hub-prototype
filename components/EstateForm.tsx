...
import { dispatch } from '@/domain/usecases/estate/getEstateUseCase';
import { EstateViewModel } from "@/domain/viewmodel/estateViewModel";
import { mapViewModelToCreateEstateInput, mapEstateToViewModel } from '@/domain/mappers/estate/estateMapper';
...
export default function EstateForm({ estateId }: EstateFormProps) {
  ...
  const handleChange = (field: keyof EstateViewModel) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    setViewModel((prev) => ({
      ...prev,
  ...
  try {
    const estate = await createEstateUseCase(mapViewModelToCreateEstateInput(viewModel), AuthorizationMode.USER_POOL);
